...
fn main() -> Result<()> {
    ...
    match opts.command {
        ...
        CliCommand::ViewLocker { base } => {
            ...
            let locker: met_voter::Locker = program.account(locker)?;
            println!("{:?}", locker);
        }
        CliCommand::ViewEscrow { base, owner } => {
            ...
            let escrow_state: met_voter::Escrow = program.account(escrow)?;
...
fn verify<C: Deref<Target = impl Signer> + Clone>(
    ...
    let locker_state: met_voter::Locker = program.account(locker)?;
...
fn new_locker<C: Deref<Target = impl Signer> + Clone>(
    ...
    let builder = program
        .request()
        .accounts(met_voter::accounts::NewLocker {
            base,
            locker,
            token_mint,
            governor,
        ...
        .args(met_voter::instruction::NewLocker {
            expiration,
            params: met_voter::LockerParams {
                max_stake_vote_multiplier,
                min_stake_duration,
                max_stake_duration,
                proposal_activation_min_votes,
...
fn new_escrow<C: Deref<Target = impl Signer> + Clone>(
    ...
    let builder = program
        ...
        .args(met_voter::instruction::NewEscrow {});
    let signature = builder.send()?;
    println!("Signature {:?}", signature);
    Ok(())
}
...
fn increase_locked_amount<C: Deref<Target = impl Signer> + Clone>(
    ...
    let builder = program
        .request()
        .accounts(met_voter::accounts::IncreaseLockedAmount {
            locker,
            escrow,
            escrow_tokens,
            source_tokens,
        ...
        .args(met_voter::instruction::IncreaseLockedAmount { amount });
    let signature = builder.send()?;
    println!("Signature {:?}", signature);
    Ok(())
}
...
fn extend_locked_duration<C: Deref<Target = impl Signer> + Clone>(
    ...
    let builder = program
        .request()
        .accounts(met_voter::accounts::ExtendLockDuration {
            locker,
            escrow,
            escrow_owner: program.payer(),
        })
        .args(met_voter::instruction::ExtendLockDuration { duration });
    let signature = builder.send()?;
    println!("Signature {:?}", signature);
    Ok(())
}
...
fn toggle_max_lock<C: Deref<Target = impl Signer> + Clone>(
    ...
    let builder = program
        .request()
        .accounts(met_voter::accounts::ToggleMaxLock {
            locker,
            escrow,
            escrow_owner: program.payer(),
        })
        .args(met_voter::instruction::ToggleMaxLock { is_max_lock });
    let signature = builder.send()?;
    println!("Signature {:?}", signature);
    Ok(())
}
...
fn withdraw<C: Deref<Target = impl Signer> + Clone>(
    ...
    let builder = program
        .request()
        .accounts(met_voter::accounts::Withdraw {
            locker,
            escrow,
            escrow_tokens,
            destination_tokens,
        ...
        .args(met_voter::instruction::Withdraw {});
    let signature = builder.send()?;
    println!("Signature {:?}", signature);
    Ok(())
}
...
fn active_proposal<C: Deref<Target = impl Signer> + Clone>(
    ...
    let builder = program
        .request()
        .accounts(met_voter::accounts::ActivateProposal {
            locker,
            escrow,
            proposal,
            escrow_owner: program.payer(),
        ...
        .args(met_voter::instruction::ActivateProposal {});
    let signature = builder.send()?;
    println!("Signature {:?}", signature);
    Ok(())
}
...
fn cast_vote<C: Deref<Target = impl Signer> + Clone>(
    ...
    instructions.push(Instruction {
        ...
        accounts: met_voter::accounts::CastVote {
            locker,
            escrow,
            vote,
            proposal,
        ...
        .to_account_metas(None),
        data: met_voter::instruction::CastVote { side }.data(),
    });
...
fn set_vote_delegate<C: Deref<Target = impl Signer> + Clone>(
    ...
    let builder = program
        .request()
        .accounts(met_voter::accounts::SetVoteDelegate {
            escrow,
            escrow_owner: program.payer(),
        })
        .args(met_voter::instruction::SetVoteDelegate { new_delegate });
    let signature = builder.send()?;
    println!("Signature {:?}", signature);
    Ok(())
}

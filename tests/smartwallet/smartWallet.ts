...
type SmartWalletState = anchor.IdlAccounts<SmartWallet>["smartWallet"];
...
describe("smartWallet", () => {
  ...
  it("transaction execution is idempotent", async () => {
    ...
    let execTxDuplicate = program.methods
      .executeTransaction()
      .accounts({
        smartWallet,
        transaction: txKey,
...
describe("Tests the smartWallet program with timelock", () => {
  ...
  it("execute tx", async () => {
    ...
    const eta = smartWalletState.minimumDelay.add(new BN(Date.now() / 1000));
    ...
    await program.methods
      .createTransactionWithTimelock(txBump, [instruction], eta)
      .accounts({
        smartWallet,
        transaction: txKey,
        proposer: ownerB.publicKey,
    ...
    let txAccount = await program.account.transaction.fetch(txKey);
    let falseStartTx = program.methods
      .executeTransaction()
      .accounts({
        smartWallet,
        transaction: txKey,
...
describe("Execute derived transaction", () => {
  ...
  it("Can transfer lamports from smart wallet", async () => {
    const index = 0;
    const [derivedWalletKey, walletBump] =
      await anchor.web3.PublicKey.findProgramAddress(
        [
    ...
    let txAccount = await program.account.transaction.fetch(txKey);
    await program.methods
      .executeTransactionDerived(new BN(index), walletBump)
      .accounts({
        smartWallet,
        transaction: txKey,
...
describe("Owner Invoker", () => {
  ...
  it("should invoke 1 of N", async () => {
    ...
    await program.methods
      .ownerInvokeInstruction(new BN(index), invokerBump, instruction)
      .accounts({
        smartWallet,
        owner: provider.wallet.publicKey,
      })
    ...
    const [subaccountInfo, bump] =
      await anchor.web3.PublicKey.findProgramAddress(
        [Buffer.from("SubaccountInfo"), invokerKey.toBuffer()],
        program.programId
      );
    await program.methods
      .createSubaccountInfo(invokerKey, smartWallet, new BN(index), {
        ["ownerInvoker"]: {},
      })
      .accounts({
        subaccountInfo,

...
import { DEFAULT_GOVERNANCE_PARAMETERS, DEFAULT_VOTE_DELAY, DEFAULT_VOTE_PERIOD } from "./constants"
...
describe("Govern", () => {
    ...
    before(async () => {
        ...
        await smartWalletProgram.methods
            ...
            .rpc();
        smartWalletState = await smartWalletProgram.account.smartWallet.fetch(smartWallet);

        //create govern

        governor = governorAddr
        await program.methods.createGovernor(locker, DEFAULT_GOVERNANCE_PARAMETERS).accounts({
            base: governBase.publicKey,
            governor,
            smartWallet,
            payer: provider.wallet.publicKey,
            systemProgram: SystemProgram.programId,
        }).signers([governBase]).rpc();
    ...
    describe("Proposal", () => {
        ...
        beforeEach("create a proposal", async () => {
            ...
            await program.methods.createProposal(0, DUMMY_INSTRUCTIONS).accounts(
                {
                    governor: governor,
                    proposal,
        ...
        it("Proposal as initialized", async () => {
            ...
            expect(proposalData.quorumVotes.toString()).to.equal(governorState["params"].quorumVotes.toString());
            expect(proposalData.queuedTransaction.toString()).to.eq(anchor.web3.PublicKey.default.toString());
            expect(proposalData.proposer.toString()).to.eq(provider.wallet.publicKey.toString());
            expect(proposalData.governor.toString()).to.eq(governor.toString());
        });

        it("Cancel a proposal", async () => {
            await program.methods.cancelProposal().accounts({
                governor,
        ...
        context("Proposal meta", () => {
            it("Cannot create proposal meta if not proposer", async () => {
                ...
                const [proposalMetaKey, bump] = await anchor.web3.PublicKey.findProgramAddress(
                    [Buffer.from("MeteoraProposalMeta"), proposalKey.toBuffer()],
                    program.programId,
                );

                const createMetaTX = await program.methods.createProposalMeta(0, title, descriptionLink).accounts({
                    proposal: proposalKey,
                    proposer: provider.wallet.publicKey,
            ...
            it("Can create proposal meta", async () => {
                ...
                const metadata = await program.account.proposalMeta.fetch(proposalMetaKey);

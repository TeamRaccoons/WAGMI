...
const BASE_KEYPAIR = (process.env as any).PERSONAL_KEYPAIR as string;
...
describe.only('Claim fee Gauge', () => {
  ...
  const voterKP = PERSONAL_KEYPAIR
    ? Keypair.fromSecretKey(bs58.decode(PERSONAL_KEYPAIR))
    : Keypair.generate();
  ...
  beforeEach('setup Minter', async () => {
    baseKP = anchor.web3.Keypair.fromSecretKey(
      new Uint8Array([
        237, 74, 208, 176, 194, 119, 36, 136, 202, 194, 99, 68, 77, 7, 141, 165,
        191, 144, 216, 120, 71, 93, 116, 232, 193, 83, 166, 189, 161, 1, 48, 0,
        43, 6, 80, 16, 121, 151, 231, 58, 204, 215, 207, 209, 177, 57, 224, 167,
        167, 157, 59, 10, 187, 119, 232, 179, 122, 115, 132, 211, 121, 13, 6,
        61,
      ])
    );

    console.log(baseKP.publicKey.toBase58());
    console.log(baseKP.secretKey.toString());

    let minterResult = await setupTokenMintAndMinter(
      baseKP,
      adminKP,
      DEFAULT_DECIMALS,
      DEFAULT_HARD_CAP
    ...
    const [minter, mBump] = await anchor.web3.PublicKey.findProgramAddress(
      [
        Buffer.from('MintWrapperMinter'),
        mintWrapper.toBuffer(),
  ...
  beforeEach('setup Governor and Voter', async () => {
    ...
    // create voter escrow
    voterEscrow = await setupVoterAndLockAmount(
      locker,
      voterKP,
      lockAmount,
      programVoter
  ...
  beforeEach('setup gauge', async () => {
    ...
    await programGauge.methods
      .createGaugeFactory(
        adminKP.publicKey,
        TEST_EPOCH_SECONDS,
        new BN(Math.floor(firstEpochStartsAt.getTime() / 1_000))
      )
      .accounts({
        base: baseKP.publicKey,
        gaugeFactory,
    ...
    await programQuarry.methods
      .setMintAuthority(gaugeFactoryAddr)
      .accounts({
        admin: adminKP.publicKey,
        rewarder,
      })
    ...
    // create quarry
    let quarryResult = await createQuarryFromAmm(
      ammPool,
      rewarder,
      adminKP,
      programQuarry
    ...
    const result = await getOrCreateVoterGauge(
      gauge,
      voterKP,
      programGauge,
      programVoter
  ...
  it('A voter claim all fee', async () => {
    ...
    var tokenAFeeBalance = await provider.connection.getTokenAccountBalance(
      tokenAFee
    );
    expect(Number(tokenAFeeBalance.value.amount)).to.deep.equal(
      (swapAAmount * AMM_FEE) / 10000
    );
    ...
    // prepare epoch
    await getOrCreateEpochGaugeVoterForCurrentEpoch(
      gauge,
      voterKP.publicKey,
      programGauge,
      programVoter
    ...
    // commit votes
    await getOrCreateEpochGaugeVoteByCurrentEpoch(
      gauge,
      voterKP.publicKey,
      programGauge,
      programVoter
    ...
    await programGauge.methods
      ...
      .rpc();
    let gaugeFactoryState = await programGauge.account.gaugeFactory.fetch(
      gaugeFactory
    );
    expect(gaugeFactoryState.currentVotingEpoch).equal(3);

    // claim some fee in epoch 2
    await claimAFeeInVotingEpoch(
      gauge,
      voterKP,
      2,
      programGauge,
    ...
    var destTokenAccountBalance =
      await provider.connection.getTokenAccountBalance(destTokenAccount);
    expect(Number(destTokenAccountBalance.value.amount)).to.deep.equal(
      (swapAAmount * AMM_FEE) / 10000
    );

    let epochGaugeVoter = await getEpochGaugeVoterForVotingEpoch(
      gauge,
      voterKP.publicKey,
      2,
      programGauge,
    ...
    let epochGaugeVoterState = await programGauge.account.epochGaugeVoter.fetch(
      epochGaugeVoter
    );
    ...
    var tokenBFeeBalance = await provider.connection.getTokenAccountBalance(
      tokenBFee
    );
    expect(Number(tokenBFeeBalance.value.amount)).to.deep.equal(
      (swapBAmount * AMM_FEE) / 10000
    );
    ...
    await programGauge.methods
      ...
      .rpc();
    gaugeFactoryState = await programGauge.account.gaugeFactory.fetch(
      gaugeFactory
    );
    expect(gaugeFactoryState.currentVotingEpoch).equal(4);

    // claim some fee in epoch 3
    await claimBFeeInVotingEpoch(
      gauge,
      voterKP,
      3,
      programGauge,
    ...
    var destTokenBAccountBalance =
      await provider.connection.getTokenAccountBalance(destTokenBAccount);
    expect(Number(destTokenBAccountBalance.value.amount)).to.deep.equal(
      (swapBAmount * AMM_FEE) / 10000
    );

    epochGaugeVoter = await getEpochGaugeVoterForVotingEpoch(
      gauge,
      voterKP.publicKey,
      3,
      programGauge,
    ...
    epochGaugeVoterState = await programGauge.account.epochGaugeVoter.fetch(
      epochGaugeVoter
    );
  ...
  it.only('2 voters share fee in epoch', async () => {
    // simulate swap to get some fee
    let swapAAmount = 1000000;
    await simulateSwapAtoB(ammPool, swapAAmount, adminKP);
    var tokenAFeeBalance = await provider.connection.getTokenAccountBalance(
    ...
    let epochGauge = await getOrCreateEpochGaugeForCurrentEpoch(
      gauge,
      programGauge
    );
    ...
    while (true) {
      ...
      if (expiry > onchainTimestamp) {
        console.log(`${expiry - onchainTimestamp} seconds until expiry`);
        await sleep(1000);
      } else {
        break;
      }
    ...
    var destTokenAccountBalance1 =
      await provider.connection.getTokenAccountBalance(destTokenAccount1);
    ...
    var destTokenAccountBalance2 =
      await provider.connection.getTokenAccountBalance(destTokenAccount2);

    console.log(
      'claimed fees of voter 1: ',
      destTokenAccountBalance1.value.amount
    );
    console.log(
      'claimed fees of voter 2: ',
      destTokenAccountBalance2.value.amount
    );

    let epochGaugeState = await programGauge.account.epochGauge.fetch(
      epochGauge
    );
    expect(
      +(
        Number(destTokenAccountBalance1.value.amount) +
        Number(destTokenAccountBalance2.value.amount) -
        epochGaugeState.tokenAFee.toNumber()
      )
    ).lessThan(2); // precision
...
async function assertEpochFee(gauge: PublicKey) {
  ...
  let feeABalance = await provider.connection.getTokenAccountBalance(
    gaugeState.tokenAFeeKey
  );
  expect(gaugeState.cummulativeTokenAFee.toNumber()).equal(
    Number(feeABalance.value.amount) +
      gaugeState.cummulativeClaimedTokenAFee.toNumber()
  );
  let feeBBalance = await provider.connection.getTokenAccountBalance(
    gaugeState.tokenBFeeKey
  );
  expect(gaugeState.cummulativeTokenBFee.toNumber()).equal(
    Number(feeBBalance.value.amount) +
      gaugeState.cummulativeClaimedTokenBFee.toNumber()
  );

  // assert fee in epoch
  let gaugeFactoryState = await programGauge.account.gaugeFactory.fetch(
    gaugeState.gaugeFactory
  );
  ...
  for (let i = 1; i <= votingEpoch; i++) {
    ...
    const epochGaugeState = await programGauge.account.epochGauge.fetchNullable(
      epochGauge
    );
    if (epochGaugeState) {

...
console.log({
  programGauge: programGauge.programId,
});
...
describe('Gauge', () => {
  ...
  beforeEach('setup Governor and Voter', async () => {
    ...
    await programVoter.methods
      .increaseLockedAmount(new BN(lockAmount))
      .accounts({
        locker,
        escrow,
        escrowTokens,
    ...
    // for 10 days
    await programVoter.methods
      .extendLockDuration(new BN(10 * 24 * 60 * 60))
      .accounts({
        locker,
        escrow,
        escrowOwner: voterKP.publicKey,
  ...
  beforeEach('setup gauge', async () => {
    ...
    const [gaugeVoterAddr, gaugeVoterBump] =
      await anchor.web3.PublicKey.findProgramAddress(
        [
          Buffer.from('GaugeVoter'),
          gaugeFactory.toBuffer(),
          voterEscrow.toBuffer(),
        ],
        programGauge.programId
      );
  ...
  describe('single gauge', () => {
    ...
    it('syncs based on voter weight', async () => {
      ...
      let newEpochGaugeVoter = await findEpochGaugeVoterByVotingEpoch(
        gauge,
        voterKP.publicKey,
        2,
        programGauge,
      ...
      let newEpochGaugeVoterState =
        await programGauge.account.epochGaugeVoter.fetch(newEpochGaugeVoter);
      ...
      expect(
        +(
          quarry2State.rewardsShare.toNumber() * 2 -
          quarryState.rewardsShare.toNumber()
        )
      ).lessThan(2);
    ...
    it('should allow closing the epoch gauge votes if the epoch has passed', async () => {
      ...
      try {
        let epochGaugeVote = await getOrCreateEpochGaugeVoteByVotingEpoch(
          gauge,
          voterKP.publicKey,
          2,
          programGauge,
  ...
  describe('revert', () => {
    it('can revert votes', async () => {
      ...
      await setVote(gauge, 50, voterKP, programGauge, programVoter);
      // prepare epoch
      let epochGaugeVoter = await getOrCreateEpochGaugeVoterForCurrentEpoch(
        gauge,
        voterKP.publicKey,
        programGauge,
        programVoter
      ...
      // commit votes
      let epochGaugeVote = await getOrCreateEpochGaugeVoteByCurrentEpoch(
        gauge,
        voterKP.publicKey,
        programGauge,
        programVoter
      ...
      let gaugeVoterState = await programGauge.account.gaugeVoter.fetch(
        gaugeVoter
      );
      ...
      let epochGaugeVoteState = await programGauge.account.epochGaugeVote.fetch(
        epochGaugeVote
      );
      ...
      epochGaugeVoteState =
        await programGauge.account.epochGaugeVote.fetchNullable(epochGaugeVote);
